...
- `UIRoot/InteractionPrompt`：UGUI 临时方案挂 `SimpleInteractionPromptSink`；Toolkit 方案在 `UIRoot/UIBridges` 挂 `InteractionPromptToolkitSink`。
...
| `Prompt Sink Provider` | 拖 UI 提示接收脚本。UGUI 拖 `SimpleInteractionPromptSink`；UI Toolkit 拖 `InteractionPromptToolkitSink` | 可以 | 交互仍可用，但没有提示 UI |
